defmodule Illume.Tools.Grep do
  ...
  scope the search. Caps matches at #{@max_lines_per_file} per file (via
  grep's own `-m`) — deliberately well under the `#{@max_total_lines}`-line
  total output cap (`cap/2`), not the same number: a single prolific file
  (a data dump, a minified bundle, or just a very common identifier) can
  otherwise consume the *entire* total budget by itself, silently hiding
  every other matching file from the result with no indication anything
  was omitted (see DECISIONS.md). Searches only the files
  `Illume.Tools.FileDiscovery.list/1` returns, `@batch_size` at a time as
  explicit `grep` arguments rather than letting `grep -r` walk the
  directory itself — keeps a single huge-file-count target from building
  one unbounded argument list.
  ...
  defp run_batches(batches, pattern) do
    Enum.reduce_while(batches, :no_matches, fn batch, acc ->
      args = ["-Hns", "-F", "-m", "#{@max_lines_per_file}", "--", pattern] ++ batch
  ...
  defp cap(output, root) do
    ...
    case Enum.split(relativized, @max_total_lines) do
      {kept, []} ->
        Enum.join(kept, "\n")
